/* eslint-disable react/prop-types */
import { FaHeart} from "react-icons/fa";
import { FaRegClock, FaStar } from "react-icons/fa6";

const ProductCard = () => {
  return (
    <>
      <div className=" max-w-sm mx-auto group rounded border-2">

        <div className="overflow-hidden relative">
          <img role="presentation" className="object-cover transition-all group-hover:scale-110 duration-700 ease-in-out w-full rounded h-56 bg-gray-500" src='https://res.cloudinary.com/dd273wlky/image/upload/v1664871457/TravelRaval/Phuket.webp' />
          <h3 className="z-10 group-hover:bg-white bg-[#00000050] text-sm font-medium text-white absolute top-4 right-5 rounded-full py-2 px-2 xs:text-xl md:text-sm flex items-center gap-2"> <FaHeart className="group-hover:text-[#FF0143]" /></h3>
        </div>

        <div className="p-6 space-y-0.5">
          {/* <div className="flex justify-between">
            <span className=" flex items-center gap-2 text-[#9ca3a9] font-medium"><FaStar className="text-[#ffa801] " /> 8.0 Superb</span>
          </div> */}
          <h3 className="text-sm font-semibold ">Electronics</h3>
          <div className="flex justify-between">
            <h3 className="text-xl  font-semibold ">Kitchen Blender</h3>
            <div className=" flex justify-between items-center pb-1">
              <h2 className=" text-[#9ca3a9] font-medium">
                <span className="text-[#FF0143] font-semibold text-xl ">$120.99</span>
              </h2>
            </div>
          </div>
          <p className=" border-b pb-6">High-quality wireless Bluetooth headphones with noise...</p>



          <div className="flex justify-between gap-6 font-medium text-[#9ca3a9] pt-3">
            <p className="flex gap-2 items-center"><FaRegClock /> 2024-08-01 </p>
            <p className="flex items-center" ><FaStar className="text-[#ffa801] " />4.1</p>
          </div>

        </div>
      </div>
      <div className=" max-w-sm mx-auto group rounded border-2">

        <div className="overflow-hidden relative">
          <img role="presentation" className="object-cover transition-all group-hover:scale-110 duration-700 ease-in-out w-full rounded h-56 bg-gray-500" src='https://res.cloudinary.com/dd273wlky/image/upload/v1664871457/TravelRaval/Phuket.webp' />
          <h3 className="z-10 group-hover:bg-white bg-[#00000050] text-sm font-medium text-white absolute top-4 right-5 rounded-full py-2 px-2 xs:text-xl md:text-sm flex items-center gap-2"> <FaHeart className="group-hover:text-[#FF0143]" /></h3>
        </div>

        <div className="p-6 space-y-0.5">
          {/* <div className="flex justify-between">
            <span className=" flex items-center gap-2 text-[#9ca3a9] font-medium"><FaStar className="text-[#ffa801] " /> 8.0 Superb</span>
          </div> */}
          <h3 className="text-sm font-semibold ">Electronics</h3>
          <div className="flex justify-between">
            <h3 className="text-xl  font-semibold ">Gaming Laptop</h3>
            <div className=" flex justify-between items-center pb-1">
              <h2 className=" text-[#9ca3a9] font-medium">
                <span className="text-[#FF0143] font-semibold text-xl ">$120.99</span>
              </h2>
            </div>
          </div>
          <p className=" border-b pb-6">High-quality wireless Bluetooth headphones with noise...</p>



          <div className="flex justify-between gap-6 font-medium text-[#9ca3a9] pt-3">
            <p className="flex gap-2 items-center"><FaRegClock /> 2024-08-01 </p>
            <p className="flex items-center" ><FaStar className="text-[#ffa801] " />4.1</p>
          </div>

        </div>
      </div>
      <div className=" max-w-sm mx-auto group rounded border-2">

        <div className="overflow-hidden relative">
          <img role="presentation" className="object-cover transition-all group-hover:scale-110 duration-700 ease-in-out w-full rounded h-56 bg-gray-500" src='https://res.cloudinary.com/dd273wlky/image/upload/v1664871457/TravelRaval/Phuket.webp' />
          <h3 className="z-10 group-hover:bg-white bg-[#00000050] text-sm font-medium text-white absolute top-4 right-5 rounded-full py-2 px-2 xs:text-xl md:text-sm flex items-center gap-2"> <FaHeart className="group-hover:text-[#FF0143]" /></h3>
        </div>

        <div className="p-6 space-y-0.5">
          {/* <div className="flex justify-between">
            <span className=" flex items-center gap-2 text-[#9ca3a9] font-medium"><FaStar className="text-[#ffa801] " /> 8.0 Superb</span>
          </div> */}
          <h3 className="text-sm font-semibold ">Electronics</h3>
          <div className="flex justify-between">
            <h3 className="text-xl  font-semibold ">Gaming Laptop</h3>
            <div className=" flex justify-between items-center pb-1">
              <h2 className=" text-[#9ca3a9] font-medium">
                <span className="text-[#FF0143] font-semibold text-xl ">$120.99</span>
              </h2>
            </div>
          </div>
          <p className=" border-b pb-6">High-quality wireless Bluetooth headphones with noise...</p>



          <div className="flex justify-between gap-6 font-medium text-[#9ca3a9] pt-3">
            <p className="flex gap-2 items-center"><FaRegClock /> 2024-08-01 </p>
            <p className="flex items-center" ><FaStar className="text-[#ffa801] " />4.1</p>
          </div>

        </div>
      </div>
      <div className=" max-w-sm mx-auto group rounded border-2">

        <div className="overflow-hidden relative">
          <img role="presentation" className="object-cover transition-all group-hover:scale-110 duration-700 ease-in-out w-full rounded h-56 bg-gray-500" src='https://res.cloudinary.com/dd273wlky/image/upload/v1664871457/TravelRaval/Phuket.webp' />
          <h3 className="z-10 group-hover:bg-white bg-[#00000050] text-sm font-medium text-white absolute top-4 right-5 rounded-full py-2 px-2 xs:text-xl md:text-sm flex items-center gap-2"> <FaHeart className="group-hover:text-[#FF0143]" /></h3>
        </div>

        <div className="p-6 space-y-0.5">
          {/* <div className="flex justify-between">
            <span className=" flex items-center gap-2 text-[#9ca3a9] font-medium"><FaStar className="text-[#ffa801] " /> 8.0 Superb</span>
          </div> */}
          <h3 className="text-sm font-semibold ">Electronics</h3>
          <div className="flex justify-between">
            <h3 className="text-xl  font-semibold ">Kitchen Blender</h3>
            <div className=" flex justify-between items-center pb-1">
              <h2 className=" text-[#9ca3a9] font-medium">
                <span className="text-[#FF0143] font-semibold text-xl ">$120.99</span>
              </h2>
            </div>
          </div>
          <p className=" border-b pb-6">High-quality wireless Bluetooth headphones with noise...</p>



          <div className="flex justify-between gap-6 font-medium text-[#9ca3a9] pt-3">
            <p className="flex gap-2 items-center"><FaRegClock /> 2024-08-01 </p>
            <p className="flex items-center" ><FaStar className="text-[#ffa801] " />4.1</p>
          </div>

        </div>
      </div>
    </>
  );
}

export default ProductCard
